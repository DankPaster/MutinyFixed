...
class CD3DFont {
	public:
		...
		CD3DFont( const char* strFontName, DWORD dwHeight, LPDIRECT3DDEVICE9EX pDevice, DWORD dwFlags = 0L );
		~CD3DFont();
		...
		HRESULT DrawTextA( float fXPos, float fYPos, DWORD dwColor, const char* strText, DWORD dwFlags = 0L, DWORD dwBackgroundColor = 0L );
		HRESULT DrawTextW(float fXPos, float fYPos, DWORD dwColor, const wchar_t* strText, DWORD dwFlags = 0L, DWORD dwBackgroundColor = 0L);
	    
		// Function to get extent of text
		HRESULT GetTextExtent( const char* strText, SIZE* pSize );
		HRESULT GetTextExtent(const wchar_t* strText, SIZE* pSize);
		...
		HRESULT InitializeDeviceObjects( LPDIRECT3DDEVICE9EX pD3DDevice );
	...
	protected:
		...
		LPDIRECT3DDEVICE9EX m_pD3DDevice; // A D3DDevice used for rendering
