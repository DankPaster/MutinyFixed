...
__inline BOOL IsCharNumeric( char tszChar ) {
	if ( ( !IsCharAlphaA( tszChar ) ) && ( IsCharAlphaNumericA( tszChar ) ) ) {
		return TRUE;
	}
	return FALSE;
}

__inline BOOL IsCharNumeric(wchar_t tszChar) {
	if ((!IsCharAlphaW(tszChar)) && (IsCharAlphaNumericW(tszChar))) {
		return TRUE;
	}
	return FALSE;
}
...
DWORD GetCustomColor(wchar_t tszColor, BYTE bAlpha) {
	return 0;
#if 0
	switch (tszColor) {
	case (L'0'): // White
...
//-----------------------------------------------------------------------------
CD3DFont::CD3DFont( const char* strFontName, DWORD dwHeight, LPDIRECT3DDEVICE9EX pDevice, DWORD dwFlags ) {
	...
	m_pFontVertices = new FONT2DVERTEX;
	m_pEffectVertices = new FONTEFFECTVERTEX;

	InitializeDeviceObjects(pDevice);
...
//-----------------------------------------------------------------------------
HRESULT CD3DFont::InitializeDeviceObjects( LPDIRECT3DDEVICE9EX pd3dDevice )
{
    HRESULT hr;
...
HRESULT CD3DFont::GetTextExtent(const wchar_t* strText, SIZE* pSize) {
	if ((strText == NULL) || (pSize == NULL))
		return E_FAIL;
...
// Desc: Draws 2D text
//-----------------------------------------------------------------------------
HRESULT CD3DFont::DrawTextA( float fXPos, float fYPos, DWORD dwColor, const char* strText, DWORD dwFlags, DWORD dwBackgroundColor ) {
    ...
    while( *strText ) {
        ...
        if ( tszChr != ( ' ' ) ) {
			...
			{
				*m_pFontVertices++ = InitFont2DVertex( D3DXVECTOR4(fXPos+0		+0.5f ,fYPos+fHeight+1.f,1.0f,1.0f), 0x9a000000, tx1, ty2 );
				*m_pFontVertices++ = InitFont2DVertex( D3DXVECTOR4(fXPos+0		+ 0.5f ,fYPos+0		+1.f,1.0f,1.0f), 0x9a000000, tx1, ty1 );
				*m_pFontVertices++ = InitFont2DVertex( D3DXVECTOR4(fXPos+fWidth + 0.5f ,fYPos+fHeight+1.f,1.0f,1.0f), 0x9a000000, tx2, ty2 );
				*m_pFontVertices++ = InitFont2DVertex( D3DXVECTOR4(fXPos+fWidth + 0.5f ,fYPos+0		+1.f,1.0f,1.0f), 0x9a000000, tx2, ty1 );
				*m_pFontVertices++ = InitFont2DVertex( D3DXVECTOR4(fXPos+fWidth + 0.5f,fYPos+fHeight+1.f,1.0f,1.0f), 0x9a000000, tx2, ty2 );
				*m_pFontVertices++ = InitFont2DVertex( D3DXVECTOR4(fXPos+0		+ 0.5f ,fYPos+0		+1.f,1.0f,1.0f), 0x9a000000, tx1, ty1 );
				m_dwUsedFontVerts += 2;
			}
...
normalText:
		...
		{
			fXPos += fWidth * 0.075f;
			//dylan added to make spaces not be quite as spaced out
		}
    }
...
HRESULT CD3DFont::DrawTextW(float fXPos, float fYPos, DWORD dwColor, const wchar_t* strText, DWORD dwFlags, DWORD dwBackgroundColor) {
	if ((m_pD3DDevice == NULL) || (strText == NULL) || (m_pEffectVertices == NULL) || (m_pFontVertices == NULL))
		return E_FAIL;
	...
	while (*strText) {
		wchar_t tszChr = *strText++;
		DWORD strCur = (DWORD)strText;
#if 0
		if (tszChr == (L'^')) {
			if (IsCharNumeric(*strText)) {
				wchar_t tszValue = *strText++;
				dwCustomColor = GetCustomColor(tszValue, (dwColor >> 24));
